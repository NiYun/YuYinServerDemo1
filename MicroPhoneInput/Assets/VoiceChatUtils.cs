...
{
    static int voiceIdId = 0;

    public static int GetVoiceIdID() {
        VoiceChatUtils.voiceIdId = VoiceChatUtils.voiceIdId + 1;
        return VoiceChatUtils.voiceIdId;
    }


    public static Int16[] ByteToHexStr(byte[] bytes)
    {
        byte[] data = bytes;
        int i = 0;
    ...
    static byte[] ALawCompress(float[] input)
    {
        ...
        for (int i = 0; i < input.Length; ++i)
        {
            ...
            if (scaled < short.MinValue)
                ...
                clamped = (short)scaled;
            // short clamped = (short)Mathf.Clamp((int)scaled, short.MinValue, short.MaxValue);
   ...
   public static float[] SpeexDecompress(NSpeex.SpeexDecoder speexDec, byte[] data, int dataLength)
    {
